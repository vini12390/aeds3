...
public class ArquivoCategoria extends Arquivo<Categoria> {
    ...
    public ArquivoCategoria() throws Exception {
        super("dados/categorias.db", Categoria.class.getConstructor()); // Mudança para "dados/categorias.db"
        arquivo = new RandomAccessFile("dados/categorias.db", "rw"); // Mudança para "dados/categorias.db"
    }
    ...
    public ArquivoCategoria(String nomeArquivo, int ordemArvore) throws Exception {
        ...
        arquivo = new RandomAccessFile("dados/categorias.db", "rw"); // Mudança para "dados/categorias.db"
        arvoreBMais = new ArvoreBMais<>(ParNomeID.class.getConstructor(), ordemArvore, "dados/ParNomeId.db"); // Mudança para "dados/ParNomeId.db"
    }
    ...
    public void delete() throws Exception {
        ...
        if (caracter == 'S') {
            ...
            ArquivoTarefa arquivoTarefa = new ArquivoTarefa("dados/Tarefas.db", 5); // Mudança para "dados/Tarefas.db"
            int quant = arquivoTarefa.qntTarefaCategoria(idCategoria);

            if (quant != 0) {
                System.out.println("Não é possível apagar a categoria, pois ela possui tarefas associadas");
            } else {
