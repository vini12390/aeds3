...
public class ArquivoRotulo extends Arquivo<Rotulo> {
    ...
    public ArquivoRotulo(String nomeArquivo, int ordemArvore) throws Exception {
        ...
        arvoreBMais = new ArvoreBMais<>(ParNomeIDRotulo.class.getConstructor(), ordemArvore, "dados/ParNomeIDRotulo.db"); // Mudança para "dados/ParNomeIDRotulo.db"
        arvoreTR = new ArvoreBMais<>(TarefaRotulo.class.getConstructor(), 5, "dados/TarefaRotulo.db"); // Mudança para "dados/TarefaRotulo.db"
    }
    ...
    public int selecionaRotulos() throws Exception {
        ...
        for (ParNomeIDRotulo item : list) {
            if (nomeRotulo.equals(item.nomeRotulo)) {
                ...
                break; // Encontrou o rótulo, sair do loop
            }
        }
        return idRotulo;
    }
    ...
    public void update() throws Exception {
        ...
        Rotulo C;
        boolean sucesso;
        System.out.println("Você deseja alterar algum rótulo? (S/N): ");
        ...
        if (caracter == 'S' || caracter == 's') {
            ...
            if (C != null) {
                System.out.println("Rótulo atual: " + C.get());
    ...
    public void delete() throws Exception {
        ...
        System.out.println("Você realmente deseja apagar algum rótulo? (S/N) ");
        ...
        if (caracter == 'S') {
            ...
            ArquivoTarefa arquivoTarefa = new ArquivoTarefa("dados/Tarefas.db", 5);
            int quant = arquivoTarefa.qntTarefaRotulo(idRotulo);

            if (quant != 0) {
                System.out.println("Não é possível apagar o rótulo, pois ele possui tarefas associadas");
            } else {
                ...
                if (resp == true) {
                    ...
                    for (int i = 0; i < ict.size(); i++) {
                        if (ict.get(i).idRotulo == idRotulo) {
                            System.out.println("Rótulo: " + nomeRotulo);
                        }
                    }

                    resp = arvoreBMais.delete(new ParNomeIDRotulo(nomeRotulo, -1));
                    if (resp == true) {
                        System.out.println("Rótulo apagado com sucesso");
                    } else {
                        System.out.println("Erro ao apagar o rótulo");
                    }
                }
            }
        }
